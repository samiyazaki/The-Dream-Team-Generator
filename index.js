    ...
    .prompt([
      {
        ...
        validate: (nameInput) => {
          if (nameInput) {
            //If else statement to check that the user inputs are letters since only a sociopath would use numbers in their child's name
            return true;
          } else {
      ...
      },
      {
        ...
        validate: (email) => {
          const valid = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(
            email
          ); // This is a validation test to check that the user has utilized certain special characters and included a valid email address
          if (valid) {
            return true;
          } else {
...
const addEmployee = () => {
  // Here we are adding the employee cards after the manager.
  console.log(` 
        -------------------------------
  ...
  return inquirer
    .prompt([
      ...
      },
      {
        ...
        validate: (nameInput) => {
          if (isNaN(nameInput)) {
            // This if else statement is checking if the values inputted are a number and if not, console log a request for the number
    ...
    .then((employeeData) => {
      ...
      if (confirmAddEmployee) {
        // This confirms if the user wants to add an employee to the array, else it provides the array as it is.
...
const writeFile = (data) => {
  fs.writeFile("./dist/index.html", data, (err) => {
    ...
    if (err) {
      ...
      console.log(
        "Your team profile has been successfully created! Please check out the index.html in the dist file."
      );
    }
  });
};
...
addManager() // This adds the first card, Manager to the array, then it adds an employee that was created and after that the team array is generated using the generateHTML function and javascript file.
  ...
  .then((pageHTML) => {
    //After the file is generated, the HTML file is written
    return writeFile(pageHTML);
  })
  .catch((err) => {
    // If there are any errors, print out the error.
    console.log(err);
  });
